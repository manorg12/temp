...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  const productMetaPrices = this.element.querySelector('.ProductMeta__PriceList');

  if (!newVariant) {
    productMetaPrices.style.display = 'none';
  } else {
    if (previousVariant && previousVariant['price'] === newVariant['price'] && previousVariant['compare_at_price'] === newVariant['compare_at_price']) {
      return; // The price has not changed, so there's no need to update the DOM
    }
...
function formatPrice(price) {
  ...
  const formattedNumericPrice = numericPrice.replace(/(\d)(?=(\d{3})+$)/g, '$1.');

  return '€' + formattedNumericPrice;
}

    productMetaPrices.innerHTML = '';

    const formattedPrice = formatPrice(newVariant['price']);
    const formattedCompareAtPrice = formatPrice(newVariant['compare_at_price']);

    if (formattedCompareAtPrice > formattedPrice) {
      productMetaPrices.innerHTML += `<span class="ProductMeta__Price Price Price--highlight Text--subdued u-h4" data-money-convertible>${formattedPrice}</span>`;
      productMetaPrices.innerHTML += `<span class="ProductMeta__Price Price Price--compareAt Text--subdued u-h4" data-money-convertible>${formattedCompareAtPrice}</span>`;
    } else {
      productMetaPrices.innerHTML += `<span class="ProductMeta__Price Price Text--subdued u-h4" data-money-convertible>${formattedPrice}</span>`;
    }
    ...
    productMetaPrices.style.display = '';
      ...
      value: function _updateInventory(newVariant) {
        ...
        var productFormInventory = this.element.querySelector('.inventoryMessage'),
            variantInventory = newVariant ? this.variantsInventories[newVariant['id']] : null;
        var backorderElement = this.element.querySelector('#backorder');
...
//       }
        ...
        if (newVariant.available) {
          if (variantInventory['inventory_quantity'] > 0) {
           productFormInventory.innerHTML = '<i class="fas fa-truck" style="margin-right:5px;"></i>Artikel sofort verfügbar';
            document.getElementById("backorder").setAttribute('value','Artikel sofort verfügbar');
          } else {
            if (variantInventory['inventory_policy'] === 'continue') {
             productFormInventory.innerHTML = '<i class="fas fa-truck" style="margin-right:5px;"></i>Lieferzeit: 8-10 Wochen';
            document.getElementById("backorder").setAttribute('value','8-10 Wochen');


            }
            else {
             productFormInventory.innerHTML = '<i class="fas fa-times-circle" style="margin-right:5px;"></i>Dieser Artikel ist leider ausverkauft';
             document.getElementById("backorder").setAttribute('value','Dieser Artikel ist leider ausverkauft');
  ...
  // Define formatPrice function within _updateAddToCartButton
function formatPrice(price) {
  const formattedPrice = new Intl.NumberFormat('de-DE', {
    style: 'currency',
    currency: 'EUR',
  ...
  if (!newVariant) {
    ...
    if (newVariant['available']) {
      ...
      newButton.innerHTML = '\n        <span>' + window.languages.productFormAddToCart + '</span>\n        <span class="Button__SeparatorDot"></span>\n        <span data-money-convertible>' + formatPrice(newVariant['price']) + '</span>\n      ';
    } else {
      newButton.setAttribute('disabled', 'disabled');
      newButton.classList.add('Button--secondary');
      newButton.removeAttribute('data-action');
  ...
  // We replace the HTML instead of editing as it prevents the CSS transition from interfering
